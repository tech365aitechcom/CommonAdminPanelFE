...
const GroupMgmt = () => {
  ...
  const fetchCategories = () => {
    ...
    axios
      .get(
        `${import.meta.env.VITE_REACT_APP_ENDPOINT}/api/category/getCategoryWithCounts`,
        {
          headers: {
  ...
  const fetchCodes = async () => {
    try {
      const res = await axios.get(
        `${import.meta.env.VITE_REACT_APP_ENDPOINT}/api/code/getAllCodes`,
        {
          headers: { Authorization: userToken, activeDB },
        },
      )
  ...
  const handleEditGroup = (group) => {
    ...
    setGroupCodes(
      group.codes.map((code) => (typeof code === 'string' ? code : code._id)),
    )
  ...
  const handleDeleteGroup = async (groupId) => {
    try {
      setIsLoading(true)
      await axios.delete(
        `${import.meta.env.VITE_REACT_APP_ENDPOINT}/api/group/deleteGroup?id=${groupId}`,
        {
          headers: { Authorization: userToken, activeDB },
        },
      )
        ...
        <div className='fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50'>
          <div className='bg-white rounded-lg p-6 shadow-lg w-[400px] relative'>
            ...
            <p className='text-sm font-bold mb-4'>
              Note: Select Codes from worst to best, like : D3, D2, D1
            </p>
            <input
              type='text'
                      ...
                      <tr key={cat?._id} className='border-t'>
                        <td className='px-3 py-1'>
                          <input
                            ...
                            checked={groupCategories.includes(
                              cat?.categoryCode,
                            )}
                            onChange={(e) => {
                              const value = e.target.value
